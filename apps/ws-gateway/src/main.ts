...
const env = loadEnv(
  BaseSchema.merge(ValkeySchema).merge(z.object({ WS_PORT: z.coerce.number().default(4000) }))
);
...
// Обновление метрик количества подписок и активных клиентов
const metricsTimer = setInterval(() => {
  ...
  wsConnectionsTotal.set(clients.length); // синхронизация с текущим значением
}, 5_000);
